...
class NewsvendorEnv(gym.Env):
    ...
    def _STEP(self, action_idx):
        ...
        self.demand = self.demand_list[self.step_count]
        inventory = self.state[5:]
        if self.lead_time == 0: # No lead time -> instant fulfillment
            inv_on_hand = order_qty
        else:
            inv_on_hand = inventory[0]
        sales = min(inv_on_hand, self.demand) * self.price
        excess_inventory = max(0, inv_on_hand - self.demand)
        short_inventory = max(0, self.demand - inv_on_hand)
